...
function computeStreak(habitId, history) {
  let streak = 0;
  let d = todayISO();

  while (history?.[habitId]?.[d]) {
    streak++;
    d = dayBefore(d);
  }
  return streak;
...
// inject streak into habit render
const _renderHabitRow = renderHabitRow;
renderHabitRow = function (habit) {
  const history = JSON.parse(localStorage.getItem("ht-history") || "{}");
  const streak = computeStreak(habit.id, history);

  const row = _renderHabitRow(habit);
  ...
  if (streak > 0) {
    ...
    row.querySelector(".habit-name")?.appendChild(badge);
  }

  return row;
};

// save completion with history
const _toggleHabitDay = toggleHabitDay;
toggleHabitDay = function (habitId, date) {
  const history = JSON.parse(localStorage.getItem("ht-history") || "{}");
  history[habitId] ??= {};
  history[habitId][date] = !history[habitId][date];
  localStorage.setItem("ht-history", JSON.stringify(history));

  _toggleHabitDay(habitId, date);
};
